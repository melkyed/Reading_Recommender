...
find_similarities <- function(matrix, columns_to_consider, similarity_metric, make_positive_similarities, k){
  ...
  if(!is.null(k) && k < nrow(similarities) - 1){ # if ALL - 1 that means we need all neigbours except that user/item
    ...
    # Function to finds k-th largest value in a vector.
...
#'
#' @param pred6 (dgCMatrix) Matrix where all predictions are stored
...
#' It contains predictions only for those rows and columns that exist in \code{pred6_indices}.
#' @param pred6_indices Indices of predictions matrix where predictions should be stored.
#' @returns Predictions matrix with added predictions (dgCMatrix)
add_predictions_to_prediction_matrix <- function(pred6, part_predictions, pred6_indices){
  ...
  pred6_indices <- all_indices[, c("row_name", "column_name")]
  part_matrix_indices <- all_indices[, c("row_index", "column_index")]
  
  if(nrow(pred6_indices) > 0){
    pred6[as.matrix(pred6_indices)] <- part_predictions[as.matrix(part_matrix_indices)]
  }
...
predict_cf <- function(ratings_matrix, predictions_indices, alg_method, normalization, similarity_metric, k, make_positive_similarities, rowchunk_size, columnchunk_size){
  ...
  pred6 <- as(sparseMatrix(i = c(), j = c(), dims = ratings_matrix@Dim, dimnames = ratings_matrix@Dimnames), "dgCMatrix")
  ...
  for(i in 1:num_column_splits){
    ...
    for(j in 1:num_row_splits){
      ...
      pred6 <- add_predictions_to_prediction_matrix(pred6, part_predictions, predictions_indices_to_consider)
    }
    
  }
  
  if(normalization){
    temp <- as(pred6, "realRatingMatrix")
    temp@normalize <- normalization_info
    pred6 <- denormalize(temp)
    pred6 <- as(pred6, "dgCMatrix")
  }
